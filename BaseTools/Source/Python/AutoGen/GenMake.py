...
\t${END}@cd $(MODULE_BUILD_DIR)\n\n''')
    ...
    def ProcessBuildTargetList(self):
        ...
        #
        for File in sorted(DepSet, key=lambda x: x.Path):
        ...
        for File in FileDependencyDict:
            ...
            NewDepSet -= DepSet
            FileDependencyDict[File] = ["$(COMMON_DEPS)"] + list(sorted(NewDepSet, key=lambda x: x.Path))
    ...
    def GetDependencyList(self, File, ForceList, SearchPathList):
        ...
        DependencyList = list(sorted(DependencySet, key=lambda x: x.Path))  # remove duplicate ones
